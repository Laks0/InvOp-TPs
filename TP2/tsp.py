...
def agregar_restricciones(prob: cplex.Cplex, instancia: InstanciaRecorridoMixto):
    ...
    # A toda ciudad se entra una vez
    for i in range(n):
        indices_x = []
        for j in range(n):
            if i == j: continue
    ...
    for i in range(n):
        ...
        prob.linear_constraints.add(lin_expr=[lhs], senses=["E"], rhs=[1], names=[f"Salgo de {i}"])
...
def armar_lp(prob: cplex.Cplex, instancia):
    ...
    prob.write('modelos/m1_'+sys.argv[1].strip().split("/")[-1]+'.lp')
