...
def testeos_rendimiento():
    ...
    tiempos = [[] for i in range(6)]

    df = pd.DataFrame(columns=["minimum_infeasibility", "automatic", "maximum_infeasibility", "pseudo_costs", "string_branching", "pseudo_reduced_costs"])

    for i in range(3):
        for varsel in range(-1, 5):
            ...
            mem_limit = lambda c: c.parameters.mip.limits.treememory.set(1024 * 6)  # 6GB

            prob._modelo.parameters.randomseed.set(seed)
            prob._modelo.parameters.timelimit.set(60 * 5) # 5 min
            prob._modelo.parameters.mip.strategy.variableselect.set(varsel)

            t0 = time.monotonic()
            prob.resolver(mem_limit)
            t1 = time.monotonic()

            objetivo, _, _ = prob.obtener_solucion()

            tiempo = t1 - t0
            tiempos[varsel+1].append(objetivo)

        df.loc[len(df)] = {
            "minimum_infeasibility": tiempos[0],
            "automatic": tiempos[1],
            "maximum_infeasibility": tiempos[2],
            "pseudo_costs": tiempos[3],
            "string_branching": tiempos[4],
            "pseudo_reduced_costs": tiempos[5],
        }
