...
def cargar_instancia():
    try:
        ...
        raise RuntimeError("El primer argumento debe ser una ruta a la instancia del problema")
...
def agregar_restricciones(prob: cplex.Cplex, instancia: InstanciaRecorridoMixto):
    ...
    for i, j in itertools.product(range(n), range(n)):
        ...
        indices.extend([VariableNameMapping.b(i,k) for k in instancia.clientes_alcanzables_por_repartidor_desde(i) - {j}])
        valores.extend([1]*n)
        lhs = [indices, valores]
        prob.linear_constraints.add(lin_expr=[lhs], senses=["G"], rhs=[0],
    ...
    for j in instancia.exclusivos:
        ...
        lhs = [indices, valores]
        prob.linear_constraints.add(lin_expr=[lhs], senses=["E"], rhs=[0],
...
def armar_lp(prob: cplex.Cplex, instancia):
    ...
    prob.write('modelos/m3_'+sys.argv[1].strip().split("/")[-1]+'.lp')
